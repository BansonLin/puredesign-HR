...
|---|---|
| 日期 | 2026-09-03 |
| 狀態 | **Phase 1 實作完成、驗收未結案**（2026-09-04）：T01–T30 的逐項交付狀態見 3.1 任務表的標記與第 7 節。CI（PR #1，最新一次全綠為 **run #18**，head `0c438aa`）七個 job 全綠——`supabase db reset` 從空庫、`pnpm db:seed --verify`、17 條 Playwright（16 passed／1 永久 skip）都在 CI 跑過；HEAD 其後推進到 `46265c3`／`bff24df`（新增兩支 e2e spec，共 19 條），這兩個 commit 的 CI 結果見第 7 節。但 **T09 完全未執行**，另有 B1／B6／B7／B8／B9 五項驗收未結案，要等 Banson 給 staging 金鑰與 Vercel 專案（7.2／7.3）才能結案 |
...
- **✅ 交付物完成，且該列「驗收條件」欄的每一句都已實測通過**（實測地點：本容器、或 GitHub Actions CI——最新一次全綠為 **run #18**，PR #1、head `0c438aa`、七個 job 全綠；引用時一律綁 head sha，見 D-70）。
- **✅ 的一條例外（凡驗收欄要求「本機跑 e2e」者）**：本容器沒有 Docker、沒有 Supabase 金鑰，起不了本機堆疊，`pnpm test:e2e` 實跑只有 `16 skipped / 1 passed`。這類句子（目前只有 T27 的「本機與 CI 連跑兩次皆綠」）以 **CI 跑同一組指令、連續兩次全綠代驗**（run #17 head `91fcdc8`、run #18 head `0c438aa`，皆 `1 skipped / 16 passed`），記在 7.1，不另計為 blocked（D-72）。
...
「實際證據」欄由 T30 於 2026-09-04 回填，只寫**真的跑過**的東西，並標明跑在哪裡：`[本機]`＝本容器（無 Docker、無 Supabase 金鑰）；`[CI]`＝GitHub Actions（PR #1）。**每次引用都綁 head sha**（D-70）：run #16（head `9f701bc`，<https://github.com/BansonLin/puredesign-HR/actions/runs/33889645588>）、run #17（head `91fcdc8`，<https://github.com/BansonLin/puredesign-HR/actions/runs/33894672568>）、**run #18＝目前最新一次全綠**（head `0c438aa`，七個 job 全 `success`，<https://github.com/BansonLin/puredesign-HR/actions/runs/33895490568>）三次都是七個 job 全綠；本欄多數格子引用的是首次證明該項的 run #16，其後兩次只加不減。本分支 HEAD 在本節寫成後推進到 `46265c3`（新增 `tests/e2e/disabled-account.spec.ts`、`need-hr.spec.ts` 兩支 spec），其後又推進到 `bff24df`；這兩個 commit 的 CI（run #19 `cancelled`、run #20 `in_progress`）**都還沒有結果可引用**——見第 7 節開頭。需要 Supabase staging 或實機才能成立的部分一律標 **blocked** 並附 7.2 的編號。

| Prompt 1 Target | 覆蓋任務 | 實際證據（2026-09-04 實測） |
|---|---|---|
| 1 骨架、ESLint、Vitest、Playwright、GitHub Actions | T01、T03、T27 | `pnpm lint` exit 0；`pnpm typecheck` exit 0；`pnpm test` 在 `TZ=UTC`／`Asia/Taipei`／`America/Los_Angeles` 三次皆 **34 檔 648 測試全綠**；`pnpm exec playwright test --list` 得 **17 tests in 6 files**（此數字綁 `0c438aa`；HEAD `46265c3` 新增兩支 spec 後為 `Total: 19 tests in 8 files`）；`.github/workflows/ci.yml` 五個 job（lint／typecheck／unit 三 TZ 矩陣／db／e2e）。靜態守門：`tests/unit/timezone-lint.test.ts`、`tests/unit/secrets-boundary.test.ts`、`tests/unit/guard-coverage.test.ts`。`[CI]` 同一組 job 也在 run #16 全綠（lint／typecheck／unit×3 TZ／db／e2e 七個 job 皆 `success`） |
...
| 8 DECISIONS、RUNBOOK | T29 | `docs/DECISIONS.md` D-01–D-72 編號連續、無重複、無空白列（`grep -o "^\| D-[0-9]*" docs/DECISIONS.md \| wc -l` = 72 列，與 7.1 同一數字）；`docs/RUNBOOK.md` 517 行（含 4.5／6.4／9.5 對指標母體的據實描述、8.1 hosted 專案設定、8.3 上線順序）；`README.md` 已更新 |
...
| `bff24df` | `disabled-account.spec.ts` 微調 ＋ `docs/DECISIONS.md` D-71 | #20 | **本節寫成時（2026-09-04 16:50 UTC）仍 `in_progress`，結果未知，不得引用**：<https://github.com/BansonLin/puredesign-HR/actions/runs/33896548226> |

因為 `91fcdc8`、`0c438aa`、`46265c3`、`bff24df` 都沒有動 `app/`、`lib/`、`supabase/`，run #16–#18 對現行程式碼仍然有效；但 `46265c3`／`bff24df` 動了 `tests/e2e/`，**新增的兩支 spec 到本節寫成時尚無任何一次綠燈紀錄**（B9 仍未解除）。**每個新 commit 都要重新看 Actions**：本表只在寫成的那一刻正確，讀到時請以 <https://github.com/BansonLin/puredesign-HR/actions> 為準。
...
| DECISIONS 已更新 | 本機 | `grep -o "^\| D-[0-9]*" docs/DECISIONS.md` | D-01–D-72 共 72 列，編號連續（`awk` 逐號比對無 GAP／DUP）、無重複、表身無空白列（`grep -c '^\|\s*\|'` → 0）；本任務追加 D-68（§13 驗收分兩類）、D-69（`.env.example` 完整性判準）、D-70（CI 與本容器兩種證據地點）、D-72（本機跑不動的 e2e 以 CI 連跑兩次代驗；「指令跑過」與「產物拿到」分開記）。D-71 由補 B9 測試的任務追加 |
...
| 375px 截圖 10 張（§13「PR 描述含手機寬度截圖」） | CI | run #18 `e2e` job 步驟 15「Upload Playwright traces and 375px screenshots」（`actions/upload-artifact@v4`，`91fcdc8` 起 `if: always()`） | `success`。job log：「With the provided path, there will be 11 files uploaded」→「Artifact `playwright-traces` has been successfully uploaded! Final size is 3974524 bytes. Artifact ID is 9945784513」＝ `test-results/mobile/` 的 10 張 PNG ＋ `test-results/.last-run.json`；artifact 未過期（`expires_at` 2026-09-11T16:36:55Z），下載頁 <https://github.com/BansonLin/puredesign-HR/actions/runs/33895490568/artifacts/9945784513>（原 B5 解除）。**本容器仍下載不到**（agent proxy 對 `blob.core.windows.net` 回 `curl: (56) CONNECT tunnel failed, response 403`），所以「已貼進 PR 描述」這一步還沒做 |
| e2e「本機與 CI 連跑兩次皆綠」（T27 驗收欄） | CI ×2 代本機 | CI `e2e` job 的 `pnpm exec playwright test`（與本機 `pnpm test:e2e` 同一組指令） | run #17（`91fcdc8`）與 run #18（`0c438aa`）連續兩次 `1 skipped` ／ `16 passed`。本容器無 Docker、無 Supabase 金鑰，`pnpm test:e2e` 實跑為 `16 skipped / 1 passed`（唯一執行的是 `mobile.spec.ts:87`），故「本機」那一半以 CI 代驗，不另計 blocked（3.1 圖例例外、D-72） |

### 7.2 Blocked（仍未驗收，缺 Supabase staging 金鑰／Vercel 專案／實機）

編號沿用先前版本：**原 B2（`supabase db reset` 從空庫）、B3（seed `--verify` 兩次一致）、B4（需登入的 15 條 e2e）已由 CI run #16 解除；原 B5（10 張 375px 截圖）已由 CI run #18 解除**（`91fcdc8` 把 artifact 上傳改成 `if: always()` 之後，綠燈的 run 也保留截圖），證據見 7.1，故不再列於下表。

B4 的 15 條在先前版本被拆錯過，正確拆解是：**flow 1／authz 6／first-login 1／rls 3／mobile（已登入）4 ＝ 15**；另有 `mobile.spec.ts:87`（未登入，本容器也跑得動）與 `placeholder.spec.ts`（永久 skip、不計覆蓋）各 1 條，`pnpm exec playwright test --list` 合計 `Total: 17 tests in 6 files`（此拆解綁 `0c438aa`；HEAD `46265c3` 新增兩支 spec 後為 `Total: 19 tests in 8 files`，新增的兩條尚無綠燈紀錄，見 B9）。注意 `authz.spec.ts:25`（「未登入開 /me/today → 導向 /login」）雖然邏輯上不需登入，仍在 `test.skip(!hasSupabaseEnv())` 的 describe 內，所以本容器跑不到它，只能靠 `curl` 佐證。

剩下的五項（B1、B6、B7、B8、B9）都還沒有任何一次實測。
...
| B9 | 兩句驗收條件先前**沒有被任何一層測試覆蓋**（不是環境問題，是覆蓋缺口）：(a) T07「`status='left'` 登入 → signOut 並導 `/login?reason=disabled`」；(b) T18「信義總監回應洪湘庭『需 HR 協助』→ R2 responded 並進 HR 介入清單」（`flow.spec.ts` 走的是「已處理」） | 測試已於 `46265c3` 補上（`tests/e2e/disabled-account.spec.ts`、`tests/e2e/need-hr.spec.ts`，`pnpm exec playwright test --list` 可見，本容器仍因無 Supabase 堆疊而 skip），但**尚無任何一次綠燈紀錄**：run #19 被後續 push `cancelled`，`bff24df` 的 run #20 在本節寫成時仍 `in_progress`。在有一次含這兩支 spec 的綠燈 run 之前，T07／T18 不改標記 | **T07**、**T18** | 看 run #20（或之後任何一次含這兩支 spec 的綠燈 e2e）：`disabled-account.spec.ts` 與 `need-hr.spec.ts` 皆 passed 即解除，T07／T18 收成 ✅ |
...
| **裁決**：B9 的兩條測試已在 `46265c3`／`bff24df` 補上（動了 `tests/**`），要不要留在 Phase 1 | 留下且有一次含這兩支 spec 的綠燈 e2e run 即可把 T07、T18 收成 ✅ | B9 |
