...
|---|---|
| 日期 | 2026-09-03 |
| 狀態 | **Phase 1 實作完成、驗收未結案**（2026-09-04）：T01–T30 的逐項交付狀態見 3.1 任務表的標記與第 7 節。CI（PR #1，最新一次全綠為 **run #21**，head `1fdd45f`）七個 job 全綠——`supabase db reset` 從空庫、`pnpm db:seed --verify`、19 條 Playwright（18 passed／1 永久 skip，含補 B9 的 `disabled-account`／`need-hr` 兩支 spec）都在 CI 跑過，各 run 與 head sha 見第 7 節。但 **T09 完全未執行**，另有 B1／B6／B7／B8 四項驗收未結案，要等 Banson 給 staging 金鑰與 Vercel 專案（7.2／7.3）才能結案 |
...
| T30 | ⏳(B1、B7、B8) §13 完成定義總驗收：staging 最終驗證、PR 描述（測試方式、截圖、staging URL、四組帳號） | `docs/PLAN.md`（勾選）、`docs/DECISIONS.md`、`.env.example` | T25、T28、T29 | 2 | CI lint／typecheck／unit（三 TZ）／db／e2e 全綠；`supabase db reset` 從空庫無錯；`pnpm db:seed --verify` 兩次一致（milestones 15、submissions 11、alerts 2）；staging `/hr` 今日交件應交數為 4（`e2e_fresh` 為 `sample` 不計，A02）；`grep -rho "process.env.[A-Z_]*" app lib supabase tests middleware.ts \| sort -u` 每個變數都在 `.env.example`；DECISIONS 已更新。用手機 LINE 內建瀏覽器在 staging preview 走完 C-3 五步（新人填日誌 → 主管看到預警並回應 → HR 看到已回應與複製摘要 → CEO 無按鈕 → 新人開 `/hr` 被擋）。PR 描述含：測試方式、T28 的 9 張 375px 截圖（T30 實查：實際為 **10 張**，多一張 `05a-manager-newcomer-drawer`，檔案在 run #18 的 `playwright-traces` artifact 內，見 7.1）、T09 的 Auth 設定截圖與 session 驗證結果、staging URL、四組測試帳號（`hung_hsiangting`／`mgr_xinyi`／`hr`／`ceo`；密碼另行非 git 管道交付）、Prompt 1 Checkpoints 的檔案變更總表與測試摘要；PLAN.md §3 逐項勾選。PR 開給人審，不合併、不推 main（§0）。 | 9 |

**估時合計：95 小時**（至 T20「第一次可在手機示範」約 66.5 小時）。

### 3.2 Target 覆蓋對照

「實際證據」欄由 T30 於 2026-09-04 回填，只寫**真的跑過**的東西，並標明跑在哪裡：`[本機]`＝本容器（無 Docker、無 Supabase 金鑰）；`[CI]`＝GitHub Actions（PR #1）。**每次引用都綁 head sha**（D-70）：run #16（head `9f701bc`，<https://github.com/BansonLin/puredesign-HR/actions/runs/33889645588>）、run #17（head `91fcdc8`，<https://github.com/BansonLin/puredesign-HR/actions/runs/33894672568>）、**run #18＝目前最新一次全綠**（head `0c438aa`，七個 job 全 `success`，<https://github.com/BansonLin/puredesign-HR/actions/runs/33895490568>）三次都是七個 job 全綠；本欄多數格子引用的是首次證明該項的 run #16，其後兩次只加不減。本分支 HEAD 在本節寫成後推進到 `46265c3`（新增 `tests/e2e/disabled-account.spec.ts`、`need-hr.spec.ts` 兩支 spec）→ `bff24df`（run #20 `failure`）→ **`1fdd45f`（run #21，七個 job 全綠，`e2e` 19 條中 `18 passed／1 skipped`）**，兩支新 spec 自此有綠燈紀錄——見第 7 節開頭。需要 Supabase staging 或實機才能成立的部分一律標 **blocked** 並附 7.2 的編號。

| Prompt 1 Target | 覆蓋任務 | 實際證據（2026-09-04 實測） |
|---|---|---|
| 1 骨架、ESLint、Vitest、Playwright、GitHub Actions | T01、T03、T27 | `pnpm lint` exit 0；`pnpm typecheck` exit 0；`pnpm test` 在 `TZ=UTC`／`Asia/Taipei`／`America/Los_Angeles` 三次皆 **34 檔 652 測試全綠**（`0c438aa` 時為 648，`app-nav.test.tsx` 補齊 `currentNavIndex` 與渲染斷言後 ＋4，D-73）；`pnpm exec playwright test --list` 得 **`Total: 19 tests in 8 files`**（綁 `1fdd45f`；`0c438aa` 時為 17 tests in 6 files）；`.github/workflows/ci.yml` 五個 job（lint／typecheck／unit 三 TZ 矩陣／db／e2e）。靜態守門：`tests/unit/timezone-lint.test.ts`、`tests/unit/secrets-boundary.test.ts`、`tests/unit/guard-coverage.test.ts`。`[CI]` 同一組 job 也在 run #16 全綠（lint／typecheck／unit×3 TZ／db／e2e 七個 job 皆 `success`） |
| 2 migrations、enum、索引、deny-all RLS、seed 可重跑＝§11 | T02、T03、T04、T06、T16 | migrations 從**空庫**一路跑到最新：本機 PostgreSQL 16 逐檔 `psql -v ON_ERROR_STOP=1 -f`，10 個檔全數成功；查得 `public` 九表、`pg_tables.rowsecurity` 九表皆 `t`、`pg_policies` **0 列**、`pg_type` 八個 enum。seed 內容以 `tests/unit/fixtures.test.ts`＋`tests/unit/seed-plan.test.ts` 對 §11 驗證；`SEED_PASSWORD=` 時 `pnpm db:seed` 以「seed 中止：SEED_PASSWORD 未設定…」exit 1（防呆可證）。`[本機]` psql 逐條實測索引與 FK 行為：六組重複鍵全部被擋（`submissions_daily_user_date_uidx`、`submissions_weekly_uidx`、`alerts_submission_id_rule_key_key`、`form_versions_one_published_idx`、`form_versions_one_draft_idx`、`form_versions_template_id_version_no_key`、`milestones_user_id_kind_key`）；軟刪除後同人同日可再建一筆（partial index 生效）；刪未被指向的 draft 版本成功；刪 active 版本後 `form_templates.active_version_id` 變 null 且該列仍在。`[CI]` `db` job 跑 `supabase start` → `supabase db reset`（空庫 → 全部 migrations）→ 查九表／RLS／0 policies，`success`；`e2e` job 跑 `pnpm db:seed --verify`（連跑兩次比對筆數）`success`。仍 blocked：對 **staging** `supabase db push`（B1） |
| 3 Auth、首次改密碼、30 天 session、登出、guard 每一格測試 | T06、T07、T08、T09、T27 | `lib/auth/session.ts:24` `SESSION_MAX_AGE_SECONDS = 60*60*24*30` 並作為 cookie `maxAge`；`app/(auth)/login/{page,actions}.tsx`、`.../change-password/{page,actions}`；`middleware.ts`。`tests/unit/guard.test.ts`「table covers every (role, action) cell of the 10×5 matrix」＋ guard/renderer 兩檔共 86 測試全綠；`tests/unit/guard-coverage.test.ts` 3 測試全綠。`[本機]` 未登入導向：`curl -D - /me/today` → `302`、`location: /login?next=%2Fme%2Ftoday`。`[CI]` 真登入與首次改密碼已在 run #16 的 `e2e` job 通過：`first-login.spec.ts`（`e2e_fresh` → 改密碼頁 → 改完落在 `/me/today`）、`authz.spec.ts` 六條、`flow.spec.ts` 三角色登入／登出串接。`status='left'` → `/login?reason=disabled` 由 `tests/e2e/disabled-account.spec.ts` 覆蓋（`46265c3` 新增、`1fdd45f` 改以 `[data-slot="alert"]` 定位），已在 head `1fdd45f` 的 `e2e` job（run #21）綠燈通過，B9 解除。仍 blocked：30 天 session 實機驗證（B6） |
| 4 lib/forms ＋ 渲染器六型、條件、必填、錯誤；只渲染 active 版 | T10、T13（六型以 `renderToStaticMarkup` 單元測試）、T14、T15 | `lib/forms/{schema,slots,resolve,validate,submit}.ts`；`components/forms/FormRenderer.tsx` 六個 `case`（`single_select`／`short_text`／`long_text`／`date`／`number`／`user_select`）對應 `components/forms/fields/` 六個檔＋`FieldError.tsx`；測試 `forms-resolve`／`forms-validate`／`forms-publish`／`forms-renderer`／`submit-log`／`today-form`（含在 652 內）。只渲染 active 版：`lib/db/queries/forms.ts` 只回傳 `form_templates.active_version_id` 指到的版本，`app/(front)/me/today/page.tsx:67,101` 以它渲染，`:142` 對舊版既有日誌另行處理 |
| 5 R1、R2、derived、提交時 upsert、重提處理、§11 全綠 | T05、T11、T12、T14、T16、T18、T23 | `TZ=Asia/Taipei pnpm exec vitest run tests/unit/rules.test.ts tests/unit/derived.test.ts tests/unit/metrics.test.ts` 全綠，含 §11 逐條：「嚴雅齡 9/3 → exactly one R1 with items 1 and 3, no R2」「洪湘庭 9/3 → exactly one R2」「Darren 9/3 → zero」「謝文心 9/3 → zero」「the four 9/2 logs → zero」；A1：「洪湘庭 R2 is overdue at 9/4 18:00 and open at 9/4 12:00」「嚴雅齡 R1 responded after 16h07m — not late」；R3：「9/4: pending at 12:00, missing at 18:00:00 sharp and at 18:30」；指標：「誤報率 0/1 = 0%」「主管回應率 1/2 = 50%」「24h 內回應率 1/2 = 50%」。重提狀態機 `tests/unit/rules-reconcile.test.ts` |
| 6 九個頁面 375px、按鈕 ≥ 44px | T07、T13、T15、T17–T26、T28 | `next build` exit 0，**13 個路由**（`/`、`/login`、`/login/change-password`、`/me/today`、`/me/history`、`/manager`、`/manager/newcomer/[id]`、`/manager/weekly`、`/hr`、`/hr/newcomer/[id]`、`/ceo`、`/api/export/newcomer/[id]`、`/_not-found`）＋ middleware。375px 稽核：`tests/e2e/mobile.spec.ts` 五條（未登入 1＋已登入 4）。`[本機]` 只跑得動未登入那條——`mobile.spec.ts:87 › 未登入：/login` **passed**，產出 `test-results/mobile/01-login.png`（本容器目前只有這 1 張，另 9 張缺）。`[CI]` run #16 的 `e2e` job 五條全過，`scrollWidth ≤ 375` 與 `[data-primary]` 高度 ≥ 44px 的斷言因此已在九個頁面上實測。**10 張截圖檔案本身**（`01-login` … `09-ceo` 加 `05a-manager-newcomer-drawer`）：`91fcdc8` 把 artifact 上傳改成 `if: always()` 後已可取得——run #18 `e2e` job 步驟 15 `success`，log「there will be 11 files uploaded」「Artifact ID is 9945784513 … Final size is 3974524 bytes」（10 張 PNG ＋ `test-results/.last-run.json`），artifact `playwright-traces` 未過期（`expires_at` 2026-09-11T16:36:55Z）：<https://github.com/BansonLin/puredesign-HR/actions/runs/33895490568/artifacts/9945784513>。**原 B5 解除**；仍未做的只剩「把圖下載後貼進 PR 描述」——本容器下載不到（agent proxy 對 `blob.core.windows.net` 回 `CONNECT tunnel failed, response 403`） |
| 7 Playwright 煙霧＋未授權被擋 | T27 | `[本機]` `pnpm exec playwright test --list` → `Total: 17 tests in 6 files`（綁 `0c438aa`；HEAD `46265c3` 後為 `19 tests in 8 files`）；`pnpm test:e2e`（未設 Supabase 變數）→ `16 skipped / 1 passed`，**真正執行並通過的只有 1 條**（`mobile.spec.ts:87 › 未登入：/login`），其餘 16 條全被 skip（含永久 skip 的 `placeholder.spec.ts`）——被 skip 的測試沒有驗證任何行為，不計入覆蓋。未登入導向另以 `curl -D - /me/today` → `302` 佐證。`[CI]` run #16 `e2e` job：`Running 17 tests using 1 worker` → `1 skipped`（placeholder）、**`16 passed (57.1s)`**，即 flow 1／authz 6／first-login 1／rls 3／mobile 5 全過，含 T18 的「回應 ≤ 20 秒」計時斷言；同一組在 run #17（`91fcdc8`）與 run #18（`0c438aa`）又各跑一次，皆 `1 skipped / 16 passed`——T27 驗收欄的「連跑兩次皆綠」以此代驗（3.1 圖例例外） |
| 8 DECISIONS、RUNBOOK | T29 | `docs/DECISIONS.md` D-01–D-74 編號連續、無重複、無空白列（`grep -o "^\| D-[0-9]*" docs/DECISIONS.md \| wc -l` = 74 列，與 7.1 同一數字）；`docs/RUNBOOK.md` 518 行（含 4.5／6.4／9.5 對指標母體的據實描述、8.1 hosted 專案設定、8.3 上線順序）；`README.md` 已更新 |
| 9 部署 staging、PR 附手機截圖 | T09、T28、T30 | **blocked**：無 Vercel 專案、無 Supabase staging 金鑰（B1）。可交付的部分已備妥：`pnpm build` 綠、`.env.example` 列齊、`mobile.spec.ts` 一跑就產 **10 張**截圖（`01-login`、`02-me-today`、`03-me-history`、`04-manager`、`05-manager-newcomer`、`05a-manager-newcomer-drawer`、`06-manager-weekly`、`07-hr`、`08-hr-newcomer`、`09-ceo`，落在 `test-results/mobile/`）——本容器只產得出 `01-login.png`，另 9 張已由 CI run #18 產出並上傳到 `playwright-traces` artifact（原 B5 解除，證據見 7.1），從該 run 下載即可貼進 PR 描述。解除條件見第 7 節 |
...
| seed 執行器 | `tsx`（devDependency，5.4）；`db:seed`＝`tsx --conditions=react-server --env-file-if-exists=.env.local supabase/seed/seed.ts` | 不用 Node 原生 type stripping：它不解析 tsconfig `@/*` alias、ESM 匯入須帶 `.ts` 副檔名，seed 匯入 `lib/rules/run.ts`、`lib/db/queries/alerts.ts` 等以 `@/` 匯入的模組會找不到檔案；`tsx` 讀 tsconfig paths，並把 Node 旗標（`--conditions`、`--env-file-if-exists`）原樣傳給 Node。`--conditions=react-server` 使 `server-only` 解析到其 `react-server` export（空模組），seed 才能 import `lib/db/admin.ts`／`lib/db/queries/*` 而不 throw（T04 第一次執行即驗證）；seed 的匯入鏈（`lib/db`、`lib/rules`、`lib/forms`、`lib/time`）不含 `react`，此條件不影響其他套件。Playwright 的 `tests/e2e/global-setup.ts` 無此條件，自建 service role client（T03 白名單）；用 `--env-file-if-exists`（非 `--env-file`）：Node 22 對缺檔的 `--env-file` 直接 exit 9，CI runner 沒有 `.env.local`（變數來自 `$GITHUB_ENV`），本機仍照常載入、語意不變（D-06） |
...
|---|---|---|---|---|
| `next` | `^15.5.0`（留 15.x） | 框架 | §3 | 15.5.25 |
| `react`、`react-dom` | `^19.1.0` | Next 15 的 React | `useActionState` 是渲染器回傳欄位錯誤的機制 | 19.2.8 |
| `@supabase/supabase-js` | `^2.50.0` | service role client、`auth.admin` | 唯一資料存取路徑（§3） | 2.115.0 |
| `@supabase/ssr` | `^0.7.0` | cookie session client、`middleware.ts` | App Router cookie 讀寫與 session 刷新的官方做法 | 0.7.0 |
| `server-only` | `^0.0.1` | `lib/db/admin.ts`、`lib/auth/guard.ts`、`lib/db/queries/*` | 誤 import 進 client component 時 build 直接失敗；純 Node 下 import 會 throw，seed 以 `--conditions=react-server` 解析到空模組（5.1），Playwright `global-setup.ts` 不 import 這些檔；Vitest 單元測試只 import 不含它的純函式模組（`lib/auth/policy.ts`、`lib/forms/submit.ts`、`lib/rules`、`lib/forms`、`lib/time`、`lib/metrics`），`vitest.config.ts` 不設 `resolve.conditions:['react-server']`——那會讓 `react` 解析到無 `useState`／`useActionState` 的 react-server 版本，T13 的 `renderToStaticMarkup` 測試反而壞掉 | 0.0.1 |
| `zod` | `^4.0.0` | `lib/forms/schema.ts`、Server Action 輸入、`settings.rules` | 5.2 | 4.5.4 |
| `date-fns` | `^4.1.0` | `lib/time` | 5.2 | 4.4.0 |
| `@date-fns/tz` | `^1.2.0` | `lib/time`（`TZDate`） | 5.2；是官方 `@date-fns/tz`，不是舊的 `date-fns-tz` | 1.5.0 |
| `lucide-react` | 0.x（shadcn 帶入） | 圖示 | 5.2 | 0.577.0 |
| `radix-ui`（或 shadcn CLI 實際寫入的 `@radix-ui/react-*`） | `^1.4.0` | sheet、radio-group、label、Slot | shadcn 元件 import 目標 | 1.6.7 |
| `class-variance-authority` | `^0.7.1` | button／badge variant | 同上 | 0.7.1 |
| `clsx` | `^2.1.0` | `cn()` | 同上 | 2.1.1 |
| `tailwind-merge` | `^3.0.0` | `cn()`（v3 才支援 Tailwind v4） | 同上 | 3.6.0 |

### 5.4 devDependencies

| 套件 | 參考版本 | 用途 | 非要不可的理由 | 實際安裝 |
|---|---|---|---|---|
| `typescript` | `^5.9.0`（留 5.x） | typecheck（`tsc --noEmit`、`strict`） | §3 | 5.9.3 |
| `@types/node` | `^22.0.0` | Node 型別 | typecheck | 22.20.1 |
| `@types/react`、`@types/react-dom` | `^19.1.0` | React 19 型別 | typecheck | 19.2.18／19.2.7 |
| `tailwindcss` | `^4.1.0` | 樣式 | §3 | 4.3.3 |
| `@tailwindcss/postcss` | `^4.1.0` | `postcss.config.mjs` 唯一 plugin | 不另列 `postcss`／`autoprefixer` | 4.3.3 |
| `tw-animate-css` | `^1.3.0` | shadcn v4 動畫（sheet 開合） | shadcn init 帶入 | 1.4.0 |
| `eslint` | `^9.0.0` | lint | §3 | 9.39.5 |
| `eslint-config-next` | `^15.5.0` | 規則 | §3 | 15.5.25 |
| `vitest` | `^4.0.0` | 單元測試（`environment: node`、alias `@`／`@seed`、`include: tests/unit/**`、`oxc.jsx.runtime='automatic'`（D-08：vitest 4 綁 Vite 8，esbuild 選項無效） 供 T13 渲染器測試；不設 `resolve.conditions`、不 alias `server-only`：單元測試對象一律是不含 `server-only` 的純函式模組——T08 `lib/auth/policy.ts`、T14／T18 `lib/forms/submit.ts`） | §3；不裝 jsdom／testing-library | 4.1.11 |
| `@playwright/test` | `^1.55.0` | e2e（375×812、chromium） | §3 | 1.56.1 |
| `tsx` | `^4.19.0` | `db:seed`（tsconfig paths、直接執行 TS、Node 旗標透傳） | 5.1：Node 原生 type stripping 不解析 `@/*`、ESM 需 `.ts` 副檔名；沒有 seed 就沒有 §11 資料與 §13 idempotent 驗證 | 4.23.13 |
| `supabase` | `^2.20.0` | CLI：`start`、`db reset`、`db push`、`gen types` | A01；§13 從空庫驗證與 CI 本機堆疊 | 2.116.0 |
...
```json
{
  ...
  "scripts": {
    ...
    "db:seed": "tsx --conditions=react-server --env-file-if-exists=.env.local supabase/seed/seed.ts"
  },
  "pnpm": {
...
- `[CI]` = GitHub Actions（PR #1）。CI runner 有 Docker，所以 `supabase start`／`db reset`／`db:seed --verify`／完整 Playwright 都在那裡跑過。四次全綠可引用（皆七個 job `success`）：**run #16**（head `9f701bc`，15:28–15:35 UTC，<https://github.com/BansonLin/puredesign-HR/actions/runs/33889645588>）、**run #17**（head `91fcdc8`，16:21–16:28 UTC，<https://github.com/BansonLin/puredesign-HR/actions/runs/33894672568>）、**run #18**（head `0c438aa`，16:30–16:37 UTC，<https://github.com/BansonLin/puredesign-HR/actions/runs/33895490568>）、**run #21＝目前最新一次全綠**（head `1fdd45f`，16:50–16:57 UTC，<https://github.com/BansonLin/puredesign-HR/actions/runs/33897332960>，`e2e` job 19 條中 `18 passed／1 skipped`）。

**引用 CI 結果必須綁 commit**（D-70）。分支近況（`git log --oneline`）與各自的 run：

| commit | 內容 | run | 結果 |
|---|---|---|---|
| `9f701bc` | `docs/DECISIONS.md` D-60–D-66 | #16 | 七個 job 全 `success` |
| `91fcdc8` | e2e artifact 上傳改 `if: always()`（好讓綠燈的 run 也留下 375px 截圖）＋ `docs/`、`.env.example` 文字，**未動 `app/`、`lib/`、`supabase/`、`tests/`** | #17 | 七個 job 全 `success`（16:28:33Z 完成） |
| `0c438aa` | 只動 `docs/PLAN.md`、`docs/DECISIONS.md` | #18 | 七個 job 全 `success`（16:37:17Z 完成）；`e2e` job `Running 17 tests using 1 worker` → `1 skipped` ／ **`16 passed (1.1m)`**，artifact `playwright-traces` 上傳成功 |
| `46265c3` | 新增 `tests/e2e/disabled-account.spec.ts`、`tests/e2e/need-hr.spec.ts`（補 B9 的兩個覆蓋缺口），未動 `app/`、`lib/`、`supabase/` | #19 | `cancelled`（被下一個 push 取代），**無結果可引用** |
| `bff24df` | `disabled-account.spec.ts` 微調 ＋ `docs/DECISIONS.md` D-71 | #20 | **`failure`**（16:41–16:49 UTC，<https://github.com/BansonLin/puredesign-HR/actions/runs/33896548226>）：`e2e` job `1 failed／1 skipped／17 passed (1.2m)`，失敗的是 `disabled-account.spec.ts:43`——`getByRole("alert")` 除了頁面的警示框，還命中 Next.js 永遠空著的 `#__next-route-announcer__`（`role="alert"`），strict mode 判定兩個元素。其餘六個 job 全 `success` |
| `1fdd45f` | `disabled-account.spec.ts` 改以 `[data-slot="alert"]` 定位（修 run #20 的 strict mode 違規），未動 `app/`、`lib/`、`supabase/` | #21 | **七個 job 全 `success`＝目前最新一次全綠**（16:50–16:57 UTC，<https://github.com/BansonLin/puredesign-HR/actions/runs/33897332960>）；`e2e` job `Running 19 tests using 1 worker` → `1 skipped` ／ **`18 passed (1.1m)`**，含 B9 的兩支新 spec |

因為 `91fcdc8`、`0c438aa`、`46265c3`、`bff24df`、`1fdd45f` 都沒有動 `app/`、`lib/`、`supabase/`，run #16–#18 對現行程式碼仍然有效；`46265c3`／`bff24df`／`1fdd45f` 動的是 `tests/e2e/`，**新增的兩支 spec 已在 run #21（head `1fdd45f`）綠燈通過，B9 解除**。**每個新 commit 都要重新看 Actions**：本表只在寫成的那一刻正確，讀到時請以 <https://github.com/BansonLin/puredesign-HR/actions> 為準。
...
| unit 綠（三時區） | 本機／CI | `TZ=UTC pnpm test`／`TZ=Asia/Taipei pnpm test`／`TZ=America/Los_Angeles pnpm test` | 三次皆 `Test Files 34 passed (34)`、`Tests 652 passed (652)`（`0c438aa` 時為 648，D-73 的 `app-nav.test.tsx` ＋4）；CI `unit (UTC)`／`unit (Asia/Taipei)`／`unit (America/Los_Angeles)` 三個 job `success` |
...
| Playwright 全部條目 | CI | `e2e` job 步驟 14 `pnpm exec playwright test`（先 `supabase start` → `db reset` → `db:seed --verify`） | **run #16（head `9f701bc`）：**`Running 17 tests using 1 worker` → `1 skipped`（`placeholder.spec.ts`，永久 skip）、**`16 passed (57.1s)`**：flow 1／authz 6／first-login 1／rls 3／mobile 5 全過（原 B4 解除）。**最新為 run #21（head `1fdd45f`）：**`Running 19 tests using 1 worker` → `1 skipped`（同一支 placeholder）、**`18 passed (1.1m)`**，多出的兩條是 `disabled-account.spec.ts`、`need-hr.spec.ts`（B9 解除） |
...
| DECISIONS 已更新 | 本機 | `grep -o "^\| D-[0-9]*" docs/DECISIONS.md` | D-01–D-74 共 74 列，編號連續（`awk` 逐號比對無 GAP／DUP）、無重複、表身無空白列（`grep -c '^\|\s*\|'` → 0）；先前任務追加 D-68（§13 驗收分兩類）、D-69（`.env.example` 完整性判準）、D-70（CI 與本容器兩種證據地點）、D-71（B9 兩支 e2e 的還原策略）、D-72（本機跑不動的 e2e 以 CI 連跑兩次代驗）。本次收尾追加 D-73（`currentNavIndex` 抽純函式＋渲染斷言，補上突變測試存活的缺口）、D-74（CSV 單引號防護的顯示副作用改為未驗證措辭）。另修好 D-33／D-46／D-48 三列 inline code 內未逸出的 `\|`（在 GitHub 表格會多切一欄使該列錯位），改後每列的未逸出分隔符皆為 5 個 |
...
| 375px 截圖 10 張（§13「PR 描述含手機寬度截圖」） | CI | run #21 `e2e` job 步驟 15「Upload Playwright traces and 375px screenshots」（`actions/upload-artifact@v4`，`91fcdc8` 起 `if: always()`） | `success`。job log：「With the provided path, there will be 11 files uploaded」→「Artifact `playwright-traces` has been successfully uploaded! Final size is 3968180 bytes. Artifact ID is 9946486451」。上傳路徑是 `test-results/` ＋ `playwright-report/`，其中**確定含 `test-results/mobile/` 的 10 張 PNG**（`mobile.spec.ts` 逐頁截圖）；第 11 個檔是什麼**沒有實際打開過，不臆測**。artifact 未過期（`expires_at` 2026-09-11T16:57:39Z），下載頁 <https://github.com/BansonLin/puredesign-HR/actions/runs/33897332960/artifacts/9946486451>（原 B5 解除）。**本容器仍下載不到**（agent proxy 對 `blob.core.windows.net` 回 `curl: (56) CONNECT tunnel failed, response 403`），所以「已貼進 PR 描述」這一步還沒做 |
| e2e「本機與 CI 連跑兩次皆綠」（T27 驗收欄） | CI ×2 代本機 | CI `e2e` job 的 `pnpm exec playwright test`（與本機 `pnpm test:e2e` 同一組指令） | run #17（`91fcdc8`）與 run #18（`0c438aa`）連續兩次 `1 skipped` ／ `16 passed`；spec 增為 19 條後，最新的 run #21（`1fdd45f`）為 `1 skipped` ／ `18 passed`。本容器無 Docker、無 Supabase 金鑰，`0c438aa` 當時 `pnpm test:e2e` 實跑為 `16 skipped / 1 passed`（唯一執行的是 `mobile.spec.ts:87`），故「本機」那一半以 CI 代驗，不另計 blocked（3.1 圖例例外、D-72） |

### 7.2 Blocked（仍未驗收，缺 Supabase staging 金鑰／Vercel 專案／實機）

編號沿用先前版本：**原 B2（`supabase db reset` 從空庫）、B3（seed `--verify` 兩次一致）、B4（需登入的 15 條 e2e）已由 CI run #16 解除；原 B5（10 張 375px 截圖）已由 CI run #18 解除**（`91fcdc8` 把 artifact 上傳改成 `if: always()` 之後，綠燈的 run 也保留截圖）；**原 B9（T07 停用帳號導向、T18「需 HR 協助」進 HR 介入清單兩個覆蓋缺口）已由 CI run #21 解除**——測試於 `46265c3` 補上，`bff24df` 的 run #20 因 `getByRole("alert")` 同時命中 Next.js 空的 `#__next-route-announcer__` 而 `failure`，`1fdd45f` 改以 `[data-slot="alert"]` 定位後於 run #21 通過（19 條 `18 passed／1 skipped`），T07／T18 收成 ✅。證據見 7.1，四項故不再列於下表。

B4 的 15 條在先前版本被拆錯過，正確拆解是：**flow 1／authz 6／first-login 1／rls 3／mobile（已登入）4 ＝ 15**；另有 `mobile.spec.ts:87`（未登入，本容器也跑得動）與 `placeholder.spec.ts`（永久 skip、不計覆蓋）各 1 條，`pnpm exec playwright test --list` 合計 `Total: 17 tests in 6 files`（此拆解綁 `0c438aa`；HEAD `1fdd45f` 為 `Total: 19 tests in 8 files`，新增的兩條已在 run #21 通過）。注意 `authz.spec.ts:25`（「未登入開 /me/today → 導向 /login」）雖然邏輯上不需登入，仍在 `test.skip(!hasSupabaseEnv())` 的 describe 內，所以本容器跑不到它，只能靠 `curl` 佐證。

剩下的四項（B1、B6、B7、B8）都還沒有任何一次實測。
...
| **裁決**：B9 的兩條測試已在 `46265c3`／`bff24df`／`1fdd45f` 補上並修好（動了 `tests/**`），要不要留在 Phase 1 | 已於 CI run #21 綠燈通過，T07、T18 收成 ✅；若 Banson 裁決不留，需把兩支 spec 移出 Phase 1 並把 T07／T18 退回 ⏳ | 已解除（原 B9） |

### 7.4 待決（送交 Banson／HR 裁決，Phase 1 未改任何程式碼）

三指標的預警母體與 A02 不一致（D-67）：`lib/metrics/rates.ts` 的 `alertPopulation()` 不看 `profiles.status`（符合 A02「事實全量計」），但 `app/(front)/hr/page.tsx` 與 `/ceo` 只對 `activeNewcomers()` 載入預警，等於**離職者的歷史預警不計**。後果：停用某人會讓誤報率／主管回應率／24h 內回應率立刻跳動。兩條路：(a) 改 `/hr`、`/ceo` 的預警查詢母體——但 A02 同時是今日交件、缺交名單、部門統計、HR 介入清單的共用假設，動它會一次影響四處，屬架構決定；(b) 正式修訂 A02 為「只計在職」。Phase 1 已在 RUNBOOK 4.5／6.4／9.5 據實描述行為。若走 (a)，`tests/unit/ceo-dashboard.test.tsx`（頁面組合的人工複本，D-52）必須同步改。
