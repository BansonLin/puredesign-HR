...
- **✅ 交付物完成，且該列「驗收條件」欄的每一句都已實測通過**（實測地點：本容器、或 GitHub Actions CI run #16——PR #1、head `9f701bc`、七個 job 全綠）。
...
「實際證據」欄由 T30 於 2026-09-04 回填，只寫**真的跑過**的東西，並標明跑在哪裡：`[本機]`＝本容器（無 Docker、無 Supabase 金鑰）；`[CI]`＝GitHub Actions CI run #16（PR #1，head `9f701bc`＝本分支 HEAD，七個 job 全綠，<https://github.com/BansonLin/puredesign-HR/actions/runs/33889645588>）。需要 Supabase staging 或實機才能成立的部分一律標 **blocked** 並附 7.2 的編號。
...
| 6 九個頁面 375px、按鈕 ≥ 44px | T07、T13、T15、T17–T26、T28 | `next build` exit 0，**13 個路由**（`/`、`/login`、`/login/change-password`、`/me/today`、`/me/history`、`/manager`、`/manager/newcomer/[id]`、`/manager/weekly`、`/hr`、`/hr/newcomer/[id]`、`/ceo`、`/api/export/newcomer/[id]`、`/_not-found`）＋ middleware。375px 稽核：`tests/e2e/mobile.spec.ts` 五條（未登入 1＋已登入 4）。`[本機]` 只跑得動未登入那條——`mobile.spec.ts:87 › 未登入：/login` **passed**，產出 `test-results/mobile/01-login.png`（本容器目前只有這 1 張，另 9 張缺）。`[CI]` run #16 的 `e2e` job 五條全過，`scrollWidth ≤ 375` 與 `[data-primary]` 高度 ≥ 44px 的斷言因此已在九個頁面上實測。仍 blocked：**10 張截圖檔案本身**（`01-login` … `09-ceo` 加 `05a-manager-newcomer-drawer`）拿不到——CI 只在失敗時上傳 `test-results/`，成功的 run 沒有 artifact（B5） |
| 7 Playwright 煙霧＋未授權被擋 | T27 | `[本機]` `pnpm exec playwright test --list` → `Total: 17 tests in 6 files`；`pnpm test:e2e`（未設 Supabase 變數）→ `16 skipped / 1 passed`，**真正執行並通過的只有 1 條**（`mobile.spec.ts:87 › 未登入：/login`），其餘 16 條全被 skip（含永久 skip 的 `placeholder.spec.ts`）——被 skip 的測試沒有驗證任何行為，不計入覆蓋。未登入導向另以 `curl -D - /me/today` → `302` 佐證。`[CI]` run #16 `e2e` job：`Running 17 tests using 1 worker` → `1 skipped`（placeholder）、**`16 passed (57.1s)`**，即 flow 1／authz 6／first-login 1／rls 3／mobile 5 全過，含 T18 的「回應 ≤ 20 秒」計時斷言 |
| 8 DECISIONS、RUNBOOK | T29 | `docs/DECISIONS.md` D-01–D-69 編號連續、無重複、無空白列（`grep -o "^\| D-[0-9]*"` 逐列比對 = 69 列）；`docs/RUNBOOK.md` 517 行（含 4.5／6.4／9.5 對指標母體的據實描述、8.1 hosted 專案設定、8.3 上線順序）；`README.md` 已更新 |
| 9 部署 staging、PR 附手機截圖 | T09、T28、T30 | **blocked**：無 Vercel 專案、無 Supabase staging 金鑰（B1）。可交付的部分已備妥：`pnpm build` 綠、`.env.example` 列齊、`mobile.spec.ts` 一跑就產 **10 張**截圖（`01-login`、`02-me-today`、`03-me-history`、`04-manager`、`05-manager-newcomer`、`05a-manager-newcomer-drawer`、`06-manager-weekly`、`07-hr`、`08-hr-newcomer`、`09-ceo`，落在 `test-results/mobile/`）——本容器只產得出 `01-login.png`，另 9 張要在有 Docker 或本機 Supabase 堆疊的機器上跑（B5）。解除條件見第 7 節 |
